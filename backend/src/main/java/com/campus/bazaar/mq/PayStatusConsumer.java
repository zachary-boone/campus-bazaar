...
public class PayStatusConsumer {
    ...
    public void checkPayStatus(PayCheckMessage message) {
        ...
        if (message.getRetryLevel() < MqConstants.PAY_MAX_RETRY) {
            ...
            log.info("[PayCheck] 未支付，{}ms 后重试(level={}): {}", MqConstants.PAY_DELAY_MS[nextLevel - 1], nextLevel, orderNo);
        } else {
            // 达到最大级别：超时关单
