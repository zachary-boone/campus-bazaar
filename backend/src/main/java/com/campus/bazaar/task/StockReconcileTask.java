...
public class StockReconcileTask {
    ...
    public void reconcileSeckillStock() {
        List<SeckillCoupon> coupons = seckillCouponMapper.selectList(
                new QueryWrapper<SeckillCoupon>().gt("end_time", java.time.LocalDateTime.now()));
        if (coupons.isEmpty()) {
            return;
        }
        int fixed = 0;
        for (SeckillCoupon sc : coupons) {
            ...
            if (val == null) {
                ...
                log.warn("[StockReconcile] 库存 key 缺失，以 DB 初始化: couponId={}, stock={}", sc.getCouponId(), dbStock);
                fixed++;
                continue;
            }
            ...
            if (redisStock < 0) {
                ...
                log.warn("[StockReconcile] 库存为负，修正为 0: couponId={}, redis={}", sc.getCouponId(), redisStock);
                fixed++;
                continue;
            }
            ...
            if (redisStock > dbStock) {
                stringRedisTemplate.opsForValue().set(key, String.valueOf(dbStock));
                log.warn("[StockReconcile] 预扣超出 DB，校准为 DB 库存: couponId={}, redis={}, db={}",
                        sc.getCouponId(), redisStock, dbStock);
                fixed++;
            }
        }
        if (fixed > 0) {
            log.warn("[StockReconcile] 对账完成，共修正 {} 个秒杀券库存", fixed);
        } else {
            log.info("[StockReconcile] 对账完成，{} 个秒杀券库存一致", coupons.size());
        }
    }
}
